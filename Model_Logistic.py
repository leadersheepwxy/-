...
def logistic_model(block, normal, ax_val, ax_test):
    c_value_range = range(1, 11)
    ...
    for c in c_value_range:
        ...
        for train_index, val_index in cv.split(X_training, y_training):
            ...
            model = LogisticRegression(random_state=i, C=c / 10)
    ...
    model = LogisticRegression(random_state=bestGroup, C=bestParm)
    ...
    log_roc_val = plot_roc_curve(model, X_val, y_val, ax=ax_val, color='blue')
    log_roc_test = plot_roc_curve(model, X_testing, y_testing, ax=ax_test, color='blue')

    # plt.plot(list(c_value_range), meanAuc_list)
    # plt.xlabel('C')
    # plt.ylabel('Accuracy')  # 通过图像选择最好的参数
    # plt.xticks(list(c_value_range), list(c_value_range))
    # plt.show()
