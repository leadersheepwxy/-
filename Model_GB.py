...
def GB_model(block, normal, ax_val, ax_test):
    learning_rate_list = [0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1]
    ...
    for learning_rate in learning_rate_list:
        ...
        for train_index, val_index in cv.split(X_training, y_training):
            ...
            model = GradientBoostingClassifier(random_state=i, learning_rate=learning_rate)
    ...
    bestValue_name = ['最佳組別:', '最佳準確率:', '最佳learning rate:']  #
    ...
    model = GradientBoostingClassifier(random_state=bestGroup, learning_rate=bestParm)
    ...
    print(ScoresChart)
    GB_roc_val = plot_roc_curve(model, X_val, y_val, ax=ax_val, color='purple')
    GB_roc_test = plot_roc_curve(model, X_testing, y_testing, ax=ax_test, color='purple')


    # plt.plot(learning_rate_list, meanAuc_list)
